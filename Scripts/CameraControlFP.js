...
function firstPersonMovement() {
    ...
    if (wDown == true) {
        controlCamera(cam, "f", delta);
    }
    if (aDown == true) {
        controlCamera(cam, "l", delta);
    }
    if (sDown == true) {
        controlCamera(cam, "b", delta);
    }
    if (dDown == true) {
        controlCamera(cam, "r", delta);
    }
    if (eDown == true) {
        controlCamera(cam, "up", delta);
    }
    if (qDown == true) {
        controlCamera(cam, "down", delta);
    }
...
function controlCamera(currCamera, direction, deltaTime) {
    ...
    var velocity = cameraSpeed * Math.min(0.05, deltaTime);
